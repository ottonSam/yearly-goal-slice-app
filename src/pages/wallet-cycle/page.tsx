...
import {
  resolveWalletCycleResponseSchema,
  type WalletExpense,
  walletExpenseListSchema,
...
export default function WalletCyclePage() {
  ...
  const [actionDialogResult, setActionDialogResult] =
    React.useState<ApiErrorResult>({
      statusCode: undefined,
      body: "",
    });
  ...
  const expenseCycleId = cycleQuery.data?.cycle.id;
  ...
  React.useEffect(() => {
    const firstError =
      cycleQuery.error ?? expensesQuery.error ?? categoriesQuery.error;
    if (!firstError) {
      return;
    }
  ...
  }, [categoriesQuery.error, cycleQuery.error, expensesQuery.error]);
